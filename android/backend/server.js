...
const server=http.createServer((req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health')return send(res,200,{status:'ok',service:'al-fahd-tv-backend'});
  if(req.method==='GET'&&req.url==='/v1/config')return send(res,200,{status:'success',data:baseConfig});
  if(req.method==='GET'&&req.url.startsWith('/v1/football/matches')){
    ...
    return fetch(`https://v3.football.api-sports.io/fixtures?date=${encodeURIComponent(date)}`,{headers:{'x-apisports-key':FOOTBALL_API_KEY}})
      .then(async upstream=>{const payload=await upstream.json();if(!upstream.ok||payload.errors&&Object.keys(payload.errors).length)throw new Error('Football provider unavailable');const matches=(payload.response||[]).map(item=>({id:item.fixture?.id,date:item.fixture?.date,status:item.fixture?.status?.long||'',elapsed:item.fixture?.status?.elapsed,league:{name:item.league?.name||'',country:item.league?.country||'',logo:item.league?.logo||''},home:{name:item.teams?.home?.name||'',logo:item.teams?.home?.logo||'',score:item.goals?.home},away:{name:item.teams?.away?.name||'',logo:item.teams?.away?.logo||'',score:item.goals?.away}}));send(res,200,{status:'success',date,matches});})
      .catch(()=>send(res,502,{status:'error',message:'Football provider unavailable'}));
  }
